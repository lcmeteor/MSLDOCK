/* MSLDOCK version 0.4 
 edited by Chao Li in 20200212
 Add a call_mslrdpso function
...
//MSLDOCK
State call_mslrdpso(/* not const */ Global_Search *global_method,
                /* not const */ Local_Search *local_method,
                const State& sInit, const int thNum,
                const unsigned int num_evals, const unsigned int pop_size,
                const int outlev,  FILE *logFile,
                const Output_pop_stats& output_pop_stats,
                Molecule * const mol,
...
// We now have a mapped and evaluated population suitable for search
    ...
    if(outlev >= LOGFORADT )
    (void)fprintf( logFile, "Beginning %s (%s), with a maximum of %u energy evaluations.\n\n",
        global_method->longname(), global_method->shortname(), num_evals); //MSLDOCK
	...
	if(generation>0 && local_method != NULL) {
          global_method->hysearch(thisPop, local_method, thNum, evaluate, outlev, logFile); //search with MSLRDPSO
        
	  if (pop_size > 1 && outlev >= LOGRUNVVV) minmeanmax( logFile, thisPop, generation, info );

     }

    // note we terminate without searching if num_evals is 0 or num_generations is 0
    // current global methods' "terminate()" are generation count or convergence based
    terminate = global_method->terminate()  || evaluate->evals() >= num_evals;
