/* MSLDOCK version 0.4 
 ...
 Some variables are now definied within the function in this file, 
   not in the ls.h, making whole program thread-safe
...
Boole Solis_Wets::SW(/* not const */ Phenotype &vector, int outlev, FILE *logFile)
{
   ...
   Real deviates[size], bias[size]; //MSLDOCK change: move from ls.h
...
Boole Pseudo_Solis_Wets::SW(/* not const */ Phenotype &vector, int outlev, FILE *logFile)
{
   ...
   Boole modified = FALSE;
	...
	//MSLDOCK changes: move from ls.h, start here
	Real deviates[size], bias[size];
	Real rho[size], lower_bound_on_rho[size];
	Real temp_rho[size];
	Real init_rho = 1.0;
	Real lb_rho = 0.01;
	Real psw_trans_scale = 1.0;
	Real psw_rot_scale = 0.05;
	Real psw_tors_scale = 0.1;
	for (j=0; j<3; j++) {
		rho[j] = init_rho * psw_trans_scale;
		lower_bound_on_rho[j] = lb_rho * psw_trans_scale;
	}
	for (; j<7; j++) {
		rho[j] = init_rho * psw_rot_scale;
		lower_bound_on_rho[j] = lb_rho * psw_rot_scale;
	}
	for (; j<size; j++) {
		rho[j] = init_rho * psw_tors_scale;
		lower_bound_on_rho[j] = lb_rho * psw_tors_scale;
	}
	//MSLDOCK changes: end here
