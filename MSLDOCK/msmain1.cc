/* MSLDOCK version v0.4 
** This file is modified for adding the MSLRDPSO search algorithm 
** See the DPF_MSLDOCK case for details
** Other modifications are all written with a note "MSLDOCK"
**
** Modified by Chao Li, 12/02/2020
*******************************************************************************/
...
//MSLDOCK
double msrd_xmin[MSRD_D_MAX], msrd_xmax[MSRD_D_MAX]; 
...
//MSLDOCK
float msrd_tvmax = 2.0;
float msrd_qvmax = 1.0;
float msrd_rvmax = DegreesToRadians(50.0);
MSRD_Options msrd_options ;
int msrd_thNum = 0; //number of sub-swarms, 0 means this value is not specified manually
...
//______________________________________________________________________________
    ...
    case DPF_PARAMETER_VERSION:
        ...
        */
        ...
        pr( logFile, "\tParameters in MSLDOCK are the same as Autodock parameter version %s.\n", autodock_parameter_version );
        break;
...
//_______________________________________________________________________________________
//20200212 MSLDOCK PARAMETER SETTINGS
//______________________________________________________________________________

    case MSRD_NSUBS:
	get1arg( line, "%*s %d", &msrd_thNum, "MSRD_NSUBS");
	pr(logFile, "Specified MSLDOCK number of sub-swarms = %d.\n", msrd_thNum);
	break;
//______________________________________________________________________________

    case MSRD_FEI:
	get1arg( line, "%*s %d", &msrd_options.FE_interval, "MSRD_FEI");
	pr(logFile, "Specified MSLDOCK FE interval evaluations = %d.\n", msrd_options.FE_interval);
	break;
//______________________________________________________________________________

    case ALPHA_START:
        get1arg( line, "%*s %lf", &msrd_options.alpha_start, "ALPHA_START");
        pr(logFile, "Specified MSLDOCK starting alpha  = %f.\n", msrd_options.alpha_start);
        break;
//______________________________________________________________________________

    case ALPHA_END:
	get1arg( line, "%*s %lf", &msrd_options.alpha_end, "ALPHA_END");
	pr(logFile, "Specified MSLDOCK ending alpha = %f.\n", msrd_options.alpha_end);
	break;
//______________________________________________________________________________

    case BETA_START:
	get1arg( line, "%*s %lf", &msrd_options.beta_start, "BETA_START");
	pr(logFile, "Specified MSLDOCK starting beta = %f.\n", msrd_options.beta_start);
	break;
//______________________________________________________________________________

    case BETA_END:
	get1arg( line, "%*s %lf", &msrd_options.beta_end, "BETA_END");
	pr(logFile, "Specified MSLDOCK ending beta = %f.\n", msrd_options.beta_end);
	break;
//______________________________________________________________________________
// parameters for mslrdpso vmax. vmin = -vmax

    case MSRD_TVMAX:
	get1arg( line, "%*s %f", &msrd_tvmax, "MSRD_TVMAX");
	pr(logFile, "Specified MSLDOCK max value in translations = %f.\n", msrd_tvmax);
	break;
//______________________________________________________________________________

    case MSRD_QVMAX:
	get1arg( line, "%*s %f", &msrd_qvmax, "MSRD_QVMAX");
	pr(logFile, "Specified MSLDOCK max value in orientations = %f.\n", msrd_qvmax);
	break;
//______________________________________________________________________________

    case MSRD_RVMAX:
	get1arg( line, "%*s %f", &msrd_rvmax, "MSRD_RVMAX");
	msrd_rvmax = DegreesToRadians(msrd_rvmax);
	pr(logFile, "Specified MSLDOCK max value in torsions = %f.\n", msrd_rvmax);
	break;
//______________________________________________________________________________

    case MSRD_REGENERATE_AT_LIMIT:
	get1arg( line, "%*s %d", &msrd_options.msrd_regenerate_at_limit, "MSRD_REGENERATE_AT_LIMIT");
	pr(logFile, "Specified MSLDOCK regenerate_at_limit = %d.\n", mkbool(msrd_options.msrd_regenerate_at_limit));
	break;
//______________________________________________________________________________


//______________________________________________________
// 20200212 MSLDOCK START
//______________________________________________________
    case DPF_MSLDOCK:
	int msldock_D; //search space dimension 
...
#ifdef _OPENMP
	omp_set_num_threads(msrd_thNum);
#endif
	pr(logFile, "For docking problems with %d torsions, number of sub-swarms of MSLDOCK = %d \n", sInit.ntor, msrd_thNum);
	
	//set GlobalSearchMethod to MSLDOCK
	if (GlobalSearchMethod != NULL) {
	        pr(logFile, "Deleting the previous settings for the MSLDOCK.\n");
	    delete GlobalSearchMethod;
            GlobalSearchMethod = NULL;
          }

                pr( logFile, "\nTotal number of torsions in system = %d \n", sInit.ntor);
                msldock_D = 7 + ntor; //Dimension D of degree of freedom for a ligand
                pr(logFile, "\nTotal number of dimension is equal to the number of Degrees of Freedom = %d\n", msldock_D);
            pr( logFile, "Number of requested MSLDOCK dockings = %d run%s\n", nruns, pl(nruns));
          ...
          if(outlev>LOGBASIC) {
                pr(logFile, "MSLDOCK tvmax = %.3f\n", msrd_tvmax);
                pr(logFile, "MSLDOCK qvmax = %.3f\n", msrd_qvmax);
                pr(logFile, "MSLDOCK rvmax = %.3f\n", msrd_rvmax);
                }

          float msrd_vmax [MSRD_D_MAX]; // TODO = float [7 + sInit.ntor];
          float msrd_vmin [MSRD_D_MAX]; // TODO = float [7 + sInit.ntor];
          //translation, quaternion, torsion
                //translation x, y, z
                for(j= 0; j < 3; j++) {
                        msrd_vmax[j] = msrd_tvmax;
                        msrd_vmin[j] = -msrd_tvmax;
                }
                // quaternion components 
                for(j=3; j < 7; j++) {
                        msrd_vmax[j] = msrd_qvmax;
                        msrd_vmin[j] = -msrd_qvmax;
                }
          // torsion part         
          for( j=7; j < 7+ sInit.ntor; j++) {
                msrd_vmax[j] = msrd_rvmax;
                msrd_vmin[j] = -msrd_rvmax;
        }

        if (outlev > 2) {
            pr( logFile, "\ncalling MSLDOCK initialiseDimension: info->lo=%f %f %f , info->hi=%f %f %f,  \n", info->lo[0], info->lo[1], info->lo[2], info->hi[0], info->hi[1], info->hi[2]);
            };

        initialiseDimension(info, msrd_xmin, msrd_xmax, msldock_D);
        if (outlev > 2) {
            pr( logFile, "\nAFTER MSLDOCK initDim trans: xmin=%f %f %f , xmax=%f %f %f,  \n", msrd_xmin[0], msrd_xmin[1], msrd_xmin[2], msrd_xmax[0], msrd_xmax[1], msrd_xmax[2]);
            pr( logFile, "\nAFTER MSLDOCK initDim quat: xmin=%f %f %f %f, xmax=%f %f %f %f,  \n", msrd_xmin[3], msrd_xmin[4], msrd_xmin[5], msrd_xmin[6], msrd_xmax[3], msrd_xmax[4], msrd_xmax[5],msrd_xmax[6]);
        };


          GlobalSearchMethod = new MultiSwarmRDPSO(
                        msrd_vmax,
                        msrd_vmin,
                        msrd_xmax,
                        msrd_xmin,
                                                ...
                                                num_generations,
                                                output_pop_stats);
        ((MultiSwarmRDPSO*)GlobalSearchMethod)->initialize(pop_size, 7+sInit.ntor, outlev, logFile);

      pr(logFile, "GlobalSearchMethod is set to MSRDPSO.\n\n");

            // set lig_center if not already set, use to center "crdpdb" ligand
            center_ligand(crdorig, !B_found_about_keyword, natom, true_ligand_atoms,
              tlist, ntor, crdpdb, lig_center, &sInit.Center, &ligand.S.Center,
              outlev>=LOGBASIC, outlev, logFile);
            // save centered crdpdb coords as crd
            for(int a=0;a<natom;a++) for(xyz=0;xyz<SPACE;xyz++)
              crd[a][xyz]=crdpdb[a][xyz];
        ...
        evaluate.compute_intermol_energy(TRUE);
           ...
           //BEGINNING of multiple runs of MSLRDPSO
           for (j = 0; j < nruns; j++)
           {
                Real eintra = 0.0;  // sum of intramolecular energy for the ligand plus that of the protein
                Real einter = 0.0; // intermolecular energy between the ligand and the protein
            ...
            if(outlev>LOGBASIC)
                ...
                printdate(logFile, 2 );
                        ...
                        runStart = times(&tms_runStart);
                        //Entrance of one MSLRDPSO Run                                 
                sHist[nconf+j] = call_mslrdpso( GlobalSearchMethod, LocalSearchMethod,
                                          sInit, msrd_thNum,
                                          num_evals, pop_size,
                                          outlev, logFile,
                                          output_pop_stats, &ligand, &evaluate,
                                          B_RandomTran0, B_RandomQuat0, B_RandomDihe0,
                                          info, FN_pop_file, end_of_branch );
                        //Finished one MSLRDPSO Run                              
                        runEnd = times(&tms_runEnd);
            pr(logFile, "Time taken for this MSLRDPSO run:\n");
            timesyshms(runEnd-runStart, &tms_runStart, &tms_runEnd, logFile);
            pr(logFile, "\n");
            (void) fflush(logFile);

	    pr(logFile, "Total number of Energy Evaluations: %u\n", evaluate.evals());
	    pr(logFile, "Total number of Generations:        %u\n", ((MultiSwarmRDPSO*)GlobalSearchMethod)->num_generations());

	    pr( logFile, "\n\n\tFINAL MSLRDPSO DOCKED STATE\n" );
	    pr( logFile, "\t____________________________________________________________\n\n\n" );
