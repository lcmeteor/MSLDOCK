...
class Eval
{
   private:
	...
	//  and so can be shared read-only by concurrent evaluations:
      //unsigned int num_evals; //commented out by MSLDOCK
      int natom, Nnb;
      int *Nnb_array;
      GridMapSetInfo *info;
      MapType *map;
      ...
      Real crd[MAX_ATOMS][SPACE], (*vt)[SPACE]; //changed by MSLDOCK
   ...
   public:
      Eval(void);
      unsigned int num_evals; //added by MSLDOCK
...

{
    ...
    //MSLDOCK
    for(int m = 0; m < MAX_ATOMS; m++)
	for(int n = 0; n < SPACE; n++)
	    crd[m][n] = init_crd[m][n];
