...
/*----------------------------------------------------------------------------*/
	...
	if (argv[1][0] != '-') {
        ...
        case 'c':
            ...
            fprintf(stderr, "\n%s: command mode is not supported in this version of msldock\n", programname );
            break;
        case 'l':
	    if(argc<=2) {
		...
		return(-1);
            ...
            if ( strindex( dock_param_fn, ".dpf") != (int) strlen(dock_param_fn) - 4){
                fprintf(stderr, "\n MSLDOCK needs the extension of the docking parameter file to be \".dpf\"");
                fprintf(stderr, "\n%s: Unsuccessful Completion.\n\n", programname);
                return(-1);
            }
        ...
        case 't':
            ...
            break;
        case 'v':
	    fprintf(stdout, "MSLDOCK version 0.4\n");
            fprintf(stdout, "\n");
            fprintf(stdout, "Updated from AutoDock 4.2.6\n");
            fprintf(stdout, "Added a multi-swarm optimization search method\n");
            fprintf(stdout, "Some files has been changed to be thread-safe\n");
            fprintf(stdout, "For more details, please see MSLDOCK_README and MSLDOCK_RELEASENOTES\n");
            fprintf(stdout, "\n");
            fprintf(stdout, "The version information of AutoDock 4.2.6 is still retained, see below\n");
            fprintf(stdout, "----------------------------------------------------------------------\n");
