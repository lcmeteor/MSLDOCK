 ...
 * MSDOCK: multi-swarm optimization algorithm for flexible ligand docking 
 *
 * The code is written by
 * Chao Li, 02/2020
 * 
 * This method is thread-safe and thus can be implemented in parallel by using OpenMP
 * Set msdock_run in .dpf file to call this function
 * Caution: this version of MSDOCK only supports the command-line usage
 * Running in serial: msdock -p file.dpf -l file.dlg
 * Running in parallel: msdock.omp -p file.dpf -l file.dlg 
...
int MultiSwarmRDPSO::hysearch(Population &Pop, Local_Search *local_method, int thNum, Eval *evaluate, int outlev, FILE * logFile)
{
	...
	// print MSDOCK options if outlev>0
	if(_Pi == NULL) {
#define bool(i)  ((i)?"true":"false")
		
		pr(logFile, "MSDOCK max_generations = %d\n", max_generations);
		pr(logFile, "MSDOCK max_evaluations = %d\n", max_evals); //generally use max_evaluations as termination condition
		pr(logFile, "MSDOCK size = %d\n", size);

		pr(logFile, "MSDOCK number of sub-swarms = %d\n", thNum);
		pr(logFile, "MSDOCK FE_interval = %d (default:30*pop_size)\n", exf);

		if(outlev>LOGBASIC){
		pr(logFile, "MSDOCK alpha_start = %.2f\n", msrd_options.alpha_start);
		pr(logFile, "MSDOCK alpha_end = %.2f\n", msrd_options.alpha_end);
		pr(logFile, "MSDOCK beta_start = %.2f\n", msrd_options.beta_start);
		pr(logFile, "MSDOCK beta_end = %.2f\n", msrd_options.beta_end);
		pr(logFile, "MSDOCK regenerate_at_limit = %s\n", bool(msrd_options.msrd_regenerate_at_limit));
		}

		_Pi = new Population(Pop); // copy Pop
		
		pop_size = Pop.num_individuals();
                pr(logFile, "MSDOCK pop_size = %d\n", pop_size); // MP debug
