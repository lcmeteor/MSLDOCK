...
** See the DPF_MSDOCK case for details
** Other modifications are all written with a note "MSDOCK"
**
...
//______________________________________________________________________________
    ...
    case DPF_PARAMETER_VERSION:
        ...
        */
        ...
        pr( logFile, "\tParameters in MSDOCK are the same as Autodock parameter version %s.\n", autodock_parameter_version );
        break;
...
// parameters for mslrdpso vmax. vmin = -vmax
	...
	pr(logFile, "Specified MSDOCK max value in translations = %f.\n", msrd_tvmax);
	break;
//______________________________________________________________________________

    case MSRD_QVMAX:
	get1arg( line, "%*s %f", &msrd_qvmax, "MSRD_QVMAX");
	pr(logFile, "Specified MSDOCK max value in orientations = %f.\n", msrd_qvmax);
	break;
//______________________________________________________________________________
	...
	pr(logFile, "Specified MSDOCK max value in torsions = %f.\n", msrd_rvmax);
	break;
//______________________________________________________________________________

    case MSRD_REGENERATE_AT_LIMIT:
	get1arg( line, "%*s %d", &msrd_options.msrd_regenerate_at_limit, "MSRD_REGENERATE_AT_LIMIT");
	pr(logFile, "Specified MSDOCK regenerate_at_limit = %d.\n", mkbool(msrd_options.msrd_regenerate_at_limit));
	break;
//______________________________________________________________________________


//______________________________________________________
// 20200212 MSDOCK START
//______________________________________________________
    case DPF_MSDOCK:
	int msdock_D; //search space dimension 
        get1arg( line, "%*s %d", &nruns,"MSDOCK" );
...
#endif
	pr(logFile, "For docking problems with %d torsions, number of sub-swarms of MSDOCK = %d \n", sInit.ntor, msrd_thNum);
	
	//set GlobalSearchMethod to MSDOCK
	if (GlobalSearchMethod != NULL) {
	        pr(logFile, "Deleting the previous settings for the MSDOCK.\n");
	    delete GlobalSearchMethod;
            GlobalSearchMethod = NULL;
          }

                pr( logFile, "\nTotal number of torsions in system = %d \n", sInit.ntor);
                msdock_D = 7 + ntor; //Dimension D of degree of freedom for a ligand
                pr(logFile, "\nTotal number of dimension is equal to the number of Degrees of Freedom = %d\n", msdock_D);
            pr( logFile, "Number of requested MSDOCK dockings = %d run%s\n", nruns, pl(nruns));
          ...
          if(outlev>LOGBASIC) {
                pr(logFile, "MSDOCK tvmax = %.3f\n", msrd_tvmax);
                pr(logFile, "MSDOCK qvmax = %.3f\n", msrd_qvmax);
                pr(logFile, "MSDOCK rvmax = %.3f\n", msrd_rvmax);
                }
        ...
        if (outlev > 2) {
            pr( logFile, "\ncalling MSDOCK initialiseDimension: info->lo=%f %f %f , info->hi=%f %f %f,  \n", info->lo[0], info->lo[1], info->lo[2], info->hi[0], info->hi[1], info->hi[2]);
            };

        initialiseDimension(info, msrd_xmin, msrd_xmax, msdock_D);
        if (outlev > 2) {
            pr( logFile, "\nAFTER MSDOCK initDim trans: xmin=%f %f %f , xmax=%f %f %f,  \n", msrd_xmin[0], msrd_xmin[1], msrd_xmin[2], msrd_xmax[0], msrd_xmax[1], msrd_xmax[2]);
            pr( logFile, "\nAFTER MSDOCK initDim quat: xmin=%f %f %f %f, xmax=%f %f %f %f,  \n", msrd_xmin[3], msrd_xmin[4], msrd_xmin[5], msrd_xmin[6], msrd_xmax[3], msrd_xmax[4], msrd_xmax[5],msrd_xmax[6]);
        };
